...
ALTER TABLE EMPLOYEE DISABLE CONSTRAINT FK_EMP_DEPT;
ALTER TABLE TICKET DISABLE CONSTRAINT FK_TICKET_EMP;
ALTER TABLE PROJECT DISABLE CONSTRAINT FK_PROJ_EMP;

-- 1. DEPARTMENT
INSERT INTO DEPARTMENT (DEPT_ID, DEPT_NAME, DEPT_LOCATION, DEPT_STATE, DEPT_BUDGET)
SELECT DEPT_ID, DEPT_NAME, DEPT_LOCATION, DEPT_STATE, DEPT_BUDGET 
FROM BAI456_22.DEPARTMENT_T
WHERE DEPT_ID NOT IN (SELECT DEPT_ID FROM DEPARTMENT);

-- 2. CLIENT
INSERT INTO CLIENT
SELECT * FROM BAI456_22.CLIENT_T
WHERE CLIENT_ID NOT IN (SELECT CLIENT_ID FROM CLIENT);

-- 3. SOFTWARE
INSERT INTO SOFTWARE
SELECT * FROM BAI456_22.SOFTWARE_T
WHERE SW_ID NOT IN (SELECT SW_ID FROM SOFTWARE);

-- 4. EMPLOYEE
INSERT INTO EMPLOYEE (
  EMP_ID, EMP_FNAME, EMP_LNAME, EMP_EMAIL,
  EMP_ROLE, EMP_HIRE_DATE, EMP_SALARY, EMP_CERT, DEPT_ID
)
SELECT 
  e.EMP_ID,
  e.EMP_FNAME,
...
JOIN BAI456_22.DEPARTMENT_T pd
  ON e.DEPT_ID = pd.DEPT_ID
JOIN DEPARTMENT d
  ON UPPER(TRIM(pd.DEPT_NAME)) = UPPER(TRIM(d.DEPT_NAME))
WHERE e.EMP_ID IN (
  SELECT DISTINCT EMP_ID 
  FROM BAI456_22.TICKET_T
  MINUS
  SELECT EMP_ID FROM EMPLOYEE
);

-- This query returns all employees whose DEPT_ID does not exist in the DEPARTMENT table.
-- It is used to detect "orphan" employee records where the foreign key reference to DEPARTMENT is invalid or missing,
-- helping verify referential integrity between EMPLOYEE and DEPARTMENT tables.
SELECT *
FROM EMPLOYEE e
WHERE NOT EXISTS (
...
-- 5. DEPARTMENT
INSERT INTO DEPARTMENT (DEPT_ID, DEPT_NAME, DEPT_LOCATION, DEPT_STATE, DEPT_BUDGET)
SELECT p.DEPT_ID, p.DEPT_NAME, p.DEPT_LOCATION, p.DEPT_STATE, p.DEPT_BUDGET
FROM BAI456_22.DEPARTMENT_T p
WHERE NOT EXISTS (
  SELECT 1 FROM DEPARTMENT d
  WHERE UPPER(TRIM(d.DEPT_NAME)) = UPPER(TRIM(p.DEPT_NAME))
);

-- 6. INVOICE Run after Client)
INSERT INTO INVOICE
SELECT * FROM BAI456_22.INVOICE_T
WHERE INV_ID NOT IN (SELECT INV_ID FROM INVOICE);

-- 7. LICENSE (Run after Client & Software)
INSERT INTO LICENSE
SELECT * FROM BAI456_22.LICENSE_T
WHERE LIC_ID NOT IN (SELECT LIC_ID FROM LICENSE);

-- 8. TICKET (Run after Client & Employee)
INSERT INTO TICKET
SELECT * FROM BAI456_22.TICKET_T
WHERE TICKET_ID NOT IN (SELECT TICKET_ID FROM TICKET);

-- 9. PROJECT (Run after Client & Employee)
INSERT INTO PROJECT
SELECT * FROM BAI456_22.PROJECT_T
WHERE PROJ_ID NOT IN (SELECT PROJ_ID FROM PROJECT);

ALTER TABLE EMPLOYEE ENABLE CONSTRAINT FK_EMP_DEPT;
ALTER TABLE TICKET ENABLE CONSTRAINT FK_TICKET_EMP;
ALTER TABLE PROJECT ENABLE CONSTRAINT FK_PROJ_EMP;
